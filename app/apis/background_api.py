...
background = ns.model('Background', {
    ...
    'elements': fields.List(fields.Nested(ns.model('BackgroundElement', {
        ...
        'is_opened' : fields.Boolean(default=False, description='배경 설정 내용')
        })), 
        description='캐릭터 설정')
})
...
post_parser.add_argument('is_opened', required=False, help='background 공개 여부')
...
class Background(Resource):
    ...
    def put(self):
        ...
        if args['content'] != background.name:
